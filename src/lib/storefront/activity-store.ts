...
export const activityStore = {
  ...
  record: (eventType: string, data: Record<string, unknown>) => {
    ...
    if (eventType === 'code_mode:execution_started') {
      mutateTurn(turnId, (turn) => {
        const isRetry = turn.calls.length > 0 || turn.logs.length > 0
        return {
          ...turn,
    ...
    if (eventType === 'code_mode:console') {
      ...
      mutateTurn(turnId, (turn) => ({ ...turn, logs: [...turn.logs, log] }))
      emit()
      return
    }
  },
...
export function snapshotUIState(state: {
  ...
  for (const [k, v] of state.nodes) nodes[k] = v
  return { rootIds: [...state.rootIds], nodes }
}
