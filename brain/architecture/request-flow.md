...
2. Server route (`src/routes/api.storefront-agent.ts`) wires `createCodeMode({ driver: createQuickJSIsolateDriver(), tools: catalogTools, getSkillBindings: createStorefrontUIBindings })` + `anthropicText` adapter + SSE.
