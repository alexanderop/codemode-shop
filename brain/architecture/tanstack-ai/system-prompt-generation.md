...
Implication: dynamic bindings (`ui_*`, `ai_ui:*`) are **not** documented automatically — that's why we hand-write `createStorefrontUIPrompt()` (`src/features/storefront/api/ui-prompt.ts`) with `declare function ui_*(...)` stubs the model can read. Skip it and the model has no idea those functions exist.
