...
Check installed skills (`.claude/skills/`) for any that match the plan's domain. **Invoke matched skills** — read their output and incorporate domain guidance into the plan.
...
**Skill changes:** If a phase involves creating or updating a skill (any file in `.claude/skills/`), the phase must instruct the implementer to use the `skill-creator` skill during that phase.
