...
Check installed skills (`.claude/skills/`) for any that match the review's domain.
...
- Missing verification (prove-it-works) — for UI changes, was the new flow actually exercised in a browser via `agent-browser` per CLAUDE.md? Type-check and tests passing is not verification.
- Unnecessarily added complexity (subtract-before-you-add)
- Parallel implementations across surfaces instead of one shared core (prefer-one-source-of-truth)
