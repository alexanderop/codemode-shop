...
State the chosen intent and template back to the user in one sentence before generating, e.g. _"Treating this as an **explainer** — using the Good Docs `concept` template as the spine."_
...
- **Preserve template structure** — Good Docs sections are load-bearing. Add blog voice _around_ them, not by deleting them.
